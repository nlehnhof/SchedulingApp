...
export async function GET(req: Request) {
  ...
  const [{ data: appointments }, { data: rules }, { data: errors }, { data: reasons }, { data: calendarRow }, { data: clientRow }] =
    await Promise.all([
      supabase
        .from('appointments')
        .select('*')
      ...
      // selected calendar now, not the client account.
      supabase
        .from('booking_calendars')
        .select('id, display_name, slug')
        .eq('id', calendar.calendarId)
        .single(),
      supabase.from('clients').select('email, tier').eq('id', client.clientId).single(),
    ]);
