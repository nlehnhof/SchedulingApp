...
// (google_refresh_token) stored on `clients`.
export async function GET(req: Request) {
  ...
  const [{ data: calendarRow, error }, { data: clientRow }] = await Promise.all([
    supabase
      .from('booking_calendars')
      .select('google_calendar_id, timezone')
      .eq('id', calendar.calendarId)
      .single(),
    supabase.from('clients').select('google_refresh_token').eq('id', client.clientId).single(),
  ]);
  ...
  try {
    const calendars = await listGoogleCalendars(clientRow.google_refresh_token);
    return NextResponse.json({
      linked: true,
      calendars,
      selected: calendarRow.google_calendar_id,
...
export async function PATCH(req: Request) {
  ...
  if (body.googleCalendarId !== undefined) {
    ...
    let googleCalendars;
    try {
      googleCalendars = await listGoogleCalendars(clientRow.google_refresh_token);
    } catch (err) {
