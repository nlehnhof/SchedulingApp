...
export default function CalendarsPage() {
  ...
  if (error || !data) return <p className="text-sm text-rose">Failed to load your calendars.</p>;
  ...
  return (
    <div className="flex max-w-2xl flex-col gap-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="font-display text-xl font-semibold text-text">Calendars</h1>
          ...
          <li key={cal.id} className="animate-fade-up">
            <Card hoverable padding="sm" className="flex h-full flex-col justify-between gap-3 text-sm">
              <div className="flex flex-col">
                ...
                {cal.slug && <span className="text-xs text-text-2">/visit/{cal.slug}</span>}
              </div>

              {confirmDeleteId === cal.id ? (
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-xs text-rose">Delete this calendar and everything on it?</span>
      ...
      <Modal open={!!renaming} onClose={() => setRenaming(null)} title="Rename calendar">
        <form onSubmit={handleRename} className="flex flex-col gap-4">
          <Input
            ...
            autoFocus
          />
          {renameError && <p className="text-sm text-rose">{renameError}</p>}
