...
export default function ExportPage() {
  ...
  return (
    <div className="flex max-w-xl flex-col gap-4">
      <h1 className="font-display text-xl font-semibold text-text">Export</h1>
      <Select
        label="Month"
        value={month}
        onChange={(e) => {
      ...
      {!confirming ? (
        ...
        <div className="flex flex-col gap-2 rounded-md border border-lume/40 bg-lume/25 p-3 text-sm">
      ...
      {status === 'queued' && <p className="text-sm text-jade">Sent — {message}</p>}
      {status === 'error' && <p className="text-sm text-rose">{message}</p>}
    </div>
  );
}
