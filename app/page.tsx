  ...
  return (
    <main className="min-h-screen bg-background">
      <section className="relative flex min-h-[560px] items-end overflow-hidden sm:min-h-[640px]">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-text-primary/85 via-text-primary/35 to-text-primary/10" />
        ...
        <div className="relative z-10 mx-auto flex w-full max-w-3xl flex-col items-center gap-5 px-6 pb-16 pt-24 text-center text-white">
          ...
          <h1 className="animate-fade-up font-serif text-4xl font-semibold tracking-tight [animation-delay:120ms] sm:text-5xl">
            Time, set aside for you.
          </h1>
          <span className="h-1 w-16 animate-fade-up rounded-full bg-accent [animation-delay:260ms]" />
          ...
          <Link
            href="/dashboard"
            className="mt-2 inline-flex animate-fade-up items-center justify-center rounded-md bg-accent px-6 py-3 text-base font-medium text-white transition-all duration-300 [animation-delay:480ms] hover:-translate-y-0.5 hover:scale-105 hover:bg-accent-hover hover:shadow-lg hover:shadow-accent/30"
          >
            Client sign in
          </Link>
        ...
        <span className="absolute bottom-4 left-1/2 z-10 hidden -translate-x-1/2 animate-bounce text-white/70 sm:block">
          ↓
        </span>
      </section>

      <section className="relative overflow-hidden border-t border-border bg-accent-soft/20">
        {/* Decorative floating glow for a bit of ambient motion behind the cards */}
        <div className="pointer-events-none absolute -right-24 -top-24 h-72 w-72 animate-float rounded-full bg-accent/20 blur-3xl" />
        <div className="pointer-events-none absolute -bottom-24 -left-16 h-72 w-72 animate-float rounded-full bg-accent-soft/40 blur-3xl [animation-delay:2s]" />
        ...
        <div className="relative mx-auto grid max-w-4xl grid-cols-1 gap-6 px-6 py-16 sm:grid-cols-2">
          {FEATURES.map((feature, i) => (
            <div
              ...
              className="group flex animate-fade-up flex-col gap-2 rounded-xl border border-border bg-surface p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-accent/40 hover:shadow-lg"
            >
              <h2 className="font-serif text-lg font-semibold text-text-primary transition-colors group-hover:text-accent">
                {feature.title}
              </h2>
      ...
      <footer className="mx-auto max-w-4xl px-6 py-10 text-center text-xs text-text-secondary">
